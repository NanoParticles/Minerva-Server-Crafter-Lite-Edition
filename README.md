...
Networking Caching - Write/Read - Minerva Server Crafter - Lite Edition has the capability for caching when requests are being made to Curseforge API for Public Modpacks. At runtime, when a user is ready to import a public modpack, it creates a MSCPakObj file on disk in the Modpacks folder. After the file is created, checksums are then calculated and stored in the properties.json. Minerva Server Crafter - Lite Edition reads the cache, however it uses the checksum value that was deterimined at the public modpack import as a search query for the MSCPakObj of the imported modpack thats in the Modpacks folder
